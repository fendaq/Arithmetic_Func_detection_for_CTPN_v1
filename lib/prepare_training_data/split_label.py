...
for file in files:
    ...
    for bbox in bbox_list:
        ...
        if len(bbox) == 8:
            xmin = int(np.floor(float(min(bbox[0], bbox[2], bbox[4], bbox[6])) / img_size[0] * re_size[0]))
            ymin = int(np.floor(float(min(bbox[1], bbox[3], bbox[5], bbox[7])) / img_size[1] * re_size[1]))
            xmax = int(np.ceil(float(max(bbox[0], bbox[2], bbox[4], bbox[6])) / img_size[0] * re_size[0]))
            ymax = int(np.ceil(float(max(bbox[1], bbox[3], bbox[5], bbox[7])) / img_size[1] * re_size[1]))
        elif len(bbox)==4:
            xmin = int(np.floor(float(bbox[0])/img_size[0] * re_size[0]))
            ymin = int(np.floor(float(bbox[1])/img_size[1] * re_size[1]))
            xmax = int(np.ceil(float(bbox[2])/img_size[0] * re_size[0]))
            ymax = int(np.ceil(float(bbox[3])/img_size[1] * re_size[1]))
        else:
            print(xml_file)
            assert 0, "bbox error"
